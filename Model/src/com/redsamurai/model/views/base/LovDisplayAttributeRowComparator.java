 ...
 */
public class LovDisplayAttributeRowComparator extends RowComparator {
    ...
    public int compareRows(Row row1, Row row2) {
        ...
        for (int j = 0; j < mSortCriteria.length; j++) {
            ...
            if (attrId < 0) {
                attrId = row1.getStructureDef().getAttributeIndexOf(sortCrit.getAttributeName());
    ...
    private Object getLovDisplayValue(Row row, Object lValue, int attrIndex) {
        ...
        AttributeDef attrDef = row.getStructureDef().getAttributeDef(attrIndex);
        if ("choice".equals(attrDef.getProperty("CONTROLTYPE")) && attrDef.getListBindingDef() != null) {


            String lovName = attrDef.getListBindingDef().getName();
            String attrName = attrDef.getName();
            String[] displayAttrs = attrDef.getListBindingDef().getListDisplayAttrNames();
            if (displayAttrs != null && displayAttrs.length > 0) {
                RowSetIterator lovRsi = ((BaseViewRowImpl) row).getListBindingRSI(attrName, lovName);
                ...
                if (keyAttrs != null && keyAttrs.length == 1 && keyAttrs[0].getName().endsWith(listKeyAttr)) {
                    Key lovKey = new Key(new Object[] { lValue });
